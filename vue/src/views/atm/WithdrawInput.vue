...
<template>
  <div class="main">
    ...
    <div class="center">
      <h1>输入金额</h1>
      <br> <br><br>
      <el-input v-model="money" placeholder="取款金额"></el-input>
      <el-card shadow="always">账户余额：{{ $store.state.money }} 元</el-card>
    </div>
    <div class="right">
      <div class="buttonDistance">
          <el-button plain @click="withdraw">确认</el-button>
      </div>
      <div class="buttonDistance">
...
export default {
  ...
  methods: {
    ...
    withdraw() {
      ...
      if (
        this.password.trim() ==
          JSON.parse(sessionStorage.getItem("user")).password && this.money.trim() > 0
      ){
        this.money = this.money.trim();
        if(this.money%100!=0){
          this.$message({
            showClose: true,
            message: "所取金额面值必须为100",
            type: "warning",});
        }
            ...
            this.$message({
            showClose: true,
            message: "单次取款金额超过限额2500，无法取款，请重试",
            type: "warning",});
        }
        else if(sumqumoney +parseInt( this.money) > 5000) {
          
          console.log(sumqumoney +this.money)
          this.$message({
            showClose: true,
            message: "取款金额超过今日限额5000,无法取款,请重试",
            type: "warning",
          });
        }
        else if (this.$store.state.money - this.money < 0) {
          this.$message({
            showClose: true,
            message: "余额不足",
            type: "warning",
        ...
        var _this = this;
        this.$axios
          .post("http://8.139.5.168:8080/BankTransfer/withdraw", data)
          .then(
            function(res) {
    ...
    withdraw() {
      ...
      if (
        ...
        if(sumqumoney +parseInt( this.money) > 20000) {
          this.$message({
            showClose: true,
            message: "取款金额超过今日限额20000,无法取款,请重试",
            type: "warning",
          });
          return;
        }
